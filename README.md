...
Проект поддерживает **автоматический деплой** через GitHub Actions. Push в `main` → автосборка и деплой на VDS.

Все переменные окружения хранятся в **GitHub Secrets** — ничего не хардкодится.

Подробная документация: **[DEPLOY.md](DEPLOY.md)**

Быстрый старт CI/CD:
1. Создайте SSH ключ для деплоя
2. Добавьте секреты в GitHub: `SSH_HOST`, `SSH_USER`, `SSH_PRIVATE_KEY`, `JWT_SECRET`, `APP_PORT`
3. Запустите workflow "Первоначальная настройка сервера"
4. Push в `main` — деплой автоматический
